<template>
    <div class="orders">
            ...
            <table class="highlight" v-for="order in orders" :key="order.id">
                ...
                <tr>
                    ...
                    <td class="status" v-if="order.status === 'pending'">
                        ...
                        <span>Wait, order in progress:&nbsp;<span><i
                                class="material-icons cached">cached</i></span></span>
                    </td>
                    <td class="status" v-if="order.status === 'canceled'">
                        <span class="title">Status:</span>&nbsp;
                        <span>Your order was rejected:&nbsp;<span><i
                                class="material-icons cached">cancel</i></span></span>
                    </td>
                    <td class="status" v-if="order.status === 'accepted'">
                        <span class="title">Status:</span>&nbsp;
                        <span>Your order is approved:&nbsp;<span><i
                                class="material-icons cached">check</i></span></span>
                    </td>
                    ...
                    <td class="status" v-if="order.status === 'pending'">
                        <button class="btn red" @click="USER_ORDER_STATUS({orderId: order.id, status: 'canceled'})">
                            Cancel
                            order
                        </button>
                    </td>
        ...
        <div v-else>
            <p class="about_order_content_text">Your order list is empty</p>
        </div>
    </div>
</template>
